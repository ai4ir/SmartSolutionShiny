...
if(dim(dfOrg)[1]>10000) {
  rndNo <- sample(1:dim(dfOrg)[1], 10000, replace=FALSE)
  dfOrg <- dfOrg[rndNo,]
}
...
Cooks Distance{data-navmenu="Summary"}
=============================================
...
```{r, CooksDistance}
labelAnnotate <- NA
gg1 <- renderScatterPlot(dfOrg, x="rStandard", y="cooksDist", color=aesList[["color"]],
                         size=aesList[["size"]], shape=aesList[["shape"]], tooltip=aesList[["tooltip"]],
                         data_id=aesList[["data_id"]], labelAnnotate=labelAnnotate) 
...
---------------------------------------------
### corrplot
```{r, Pairs, out.height="80%", out.width="80%", fig.height=9, fig.width=15}
if(dim(dfOrg)[1]>1000) {
  dfPairs <- dfOrg[sample(dim(dfOrg)[1],1000),] #### 추가기능 ####
} else {
  dfPairs <- dfOrg
}
...
M <- dfPairs[,c(aesList[["y"]], aesList[["x"]])] #%>% pairs()
M <- cor(M)
corrplot(M, col=col(3), method = "number",type ="upper", addCoef.col = "red",tl.col = "black", order = "FPC")
```
...
summary(selModel)
var <- varImp(selModel)
pie(var[,"Overall"], labels = row.names(var),main = "변수중요비율(%)") #### 추가기능 ####

var[,"varImp"] <- rownames(var)
var <- var[c(order(var$Overall, decreasing = TRUE)), c(2,1)]
row.names(var) <- NULL
var

```

SummaryModel2{data-navmenu="Summary"}
=============================================
...
### 수정
varImportance <- varImp(selModel)
#pct <- round((varImportance$Overall/sum(varImportance$Overall)) * 100, 1)
#pie3D(varImportance[,"Overall"], labels = paste(row.names(varImportance), "\n", paste(pct,"%",sep = "")), labelcex = 0.8, main = "변수중요비율(%)", explode = 0.05, radius = 0.9, theta = pi/ 5)
...
plotRelWeights(selModel)

#varImportance[,"varImp"] <- rownames(varImportance)
#varImpRank <- varImportance[order(varImportance$Overall, decreasing = TRUE),]
#varImpRank <- varImpRank[,-varImpRank$Overall]
#as.data.frame(varImpRank)
###
...
renderHistogram <- function(xVarNo) {
  ...
  if(!is.null(attr(dfGraph[,numVarHisto[xVarNo]], "validMin")) 
     & !is.na(attr(dfGraph[,numVarHisto[xVarNo]], "validMin"))) 
  ...
  if(!is.null(attr(dfGraph[,numVarHisto[xVarNo]], "validMax")) 
     & !is.na(attr(dfGraph[,numVarHisto[xVarNo]], "validMax"))) 
...
renderHistogramSet <- function(xVarNo) {
  ...
  if(!is.na(numVarHisto[xVarNo])) gg1 <- renderHistogram(xVarNo)
  if(!is.na(numVarHisto[xVarNo+1])) gg2 <- renderHistogram(xVarNo+1)
  if(!is.na(numVarHisto[xVarNo+2])) gg3 <- renderHistogram(xVarNo+2)
  if(!is.na(numVarHisto[xVarNo+3])) gg4 <- renderHistogram(xVarNo+3)
