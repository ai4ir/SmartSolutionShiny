---
title: "ViolinOneX with Cluster"
author: "뿌리 융합솔루션팀(010-5508-7437)"
output:
...
renderViolinPlot <- function(x, y, color=NULL, size=NULL, shape=NULL, tooltip = NULL, data_id=NULL) {
  ...
  # dfGraph <- curSampleExplore
    ...
    # labelAnnotate <- NULL
    labelAnnotate <- paste0("pValue(%) : " , round(aesList[["pValueVector"]][y], digits=2))
...
renderViolinPlotSet <- function(varNo) {
  # xVarNo <- 1
  gg1 <- NULL; gg2 <- NULL; gg3 <- NULL; gg4 <- NULL;
  if(!is.na(aesList[["y"]][varNo]))
    gg1 <- renderViolinPlot(x=aesList[["x"]], y=aesList[["y"]][varNo], color=aesList[["color"]][1])
  if(!is.na(aesList[["y"]][varNo+1]))
    gg2 <- renderViolinPlot(x=aesList[["x"]], y=aesList[["y"]][varNo+1], color=aesList[["color"]][1])
  if(!is.na(aesList[["y"]][varNo+2]))
    gg3 <- renderViolinPlot(x=aesList[["x"]], y=aesList[["y"]][varNo+2], color=aesList[["color"]][1])
  if(!is.na(aesList[["y"]][varNo+3]))
    gg4 <- renderViolinPlot(x=aesList[["x"]], y=aesList[["y"]][varNo+3], color=aesList[["color"]][1])
